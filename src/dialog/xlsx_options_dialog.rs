...
impl XlsxOptionsDialog {
    ...
    fn build_instructions_from_config(&self) -> String {
        ...
        // Handle XlsxOptionsDialog-specific actions  
        if let Some(dialog_bindings) = self.config.keybindings.0.get(&crate::config::Mode::XlsxOptionsDialog) {
            for (key_seq, action) in dialog_bindings {
                match action {
                    crate::action::Action::OpenXlsxFileBrowser => {
                        segments.push(format!("{}: Browse Files", fmt_sequence(key_seq)));
                    }
                    crate::action::Action::PasteFilePath => {
                        segments.push(format!("{}: Paste Path", fmt_sequence(key_seq)));
                    }
                    crate::action::Action::ToggleWorksheetLoad => {
                        segments.push(format!("{}: Toggle Load", fmt_sequence(key_seq)));
                    }
                    _ => {}
                }
            }
